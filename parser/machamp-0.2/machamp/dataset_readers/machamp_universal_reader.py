...
class MachampUniversalReader(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        if not (
            (is_train and not is_dev and not is_test)
            or (not is_train and is_dev and not is_test)
            or (not is_train and not is_dev and is_test)
        ):
            print('file_path:', file_path)
            assert os.path.isfile(file_path)
        
        assert 1 == len(self.datasets), \
            'len(self.datasets): '+str(len(self.datasets))
        for dataset in self.datasets:
            
            assert 'difficulty_function' in self.datasets[dataset]
            assert 'competence_function' in self.datasets[dataset]
            assert 'domain'          not in self.datasets[dataset]
            assert 'proportion'      not in self.datasets[dataset]
            assert 'fold'            not in self.datasets[dataset]
            
            difficulty_function = self.datasets[dataset]['difficulty_function']
            competence_function = self.datasets[dataset]['competence_function']
            #domain     = self.datasets[dataset]['domain']
            #proportion = self.datasets[dataset]['proportion']
            #fold       = self.datasets[dataset]['fold']
            
            print('is_train           :', is_train           , file=sys.stderr) 
            print('is_dev             :', is_dev             , file=sys.stderr) 
            print('is_test            :', is_test            , file=sys.stderr) 
            print('difficulty_function:', difficulty_function, file=sys.stderr)
            print('competence_function:', competence_function, file=sys.stderr)
            ...
            #    file.close()
            #    if 5 < tell:
            #        sys.stderr.write('second batch read as expected: exiting\n')
            #        sys.exit(tell)
    ...
    def read_sequence(
        ...
        conllu_file = open(path, 'r')
        list_of_conllu_objs = conllu.parse(conllu_file.read())
        conllu_file.close()
        
        for conllu_obj, (sent, full_data) \
        in zip(list_of_conllu_objs, seqs2data(path, self.do_lowercase)):
            ...
            _instance = self.text_to_instance(
                sent_tasks,
                full_data,
                col_idxs,
                is_train,
    ...
    def text_to_instance(
        ...
        sent_tasks['is_train']   = is_train
        sent_tasks['no_dev']     = False
        #sent_tasks['conllu_obj'] = conllu_obj
        #sent_tasks['domain']     = domain
        #sent_tasks['proportion'] = proportion
        #sent_tasks['fold']       = fold
        
        fields["metadata"] = MetadataField(sent_tasks)
        
        #return Instance(fields)
        
        class MyInstance(allennlp.data.instance.Instance):
            def __init__(
                self,
                fields              : typing.MutableMapping[str, allennlp.data.fields.field.Field],
                conllu_obj          : conllu.models.TokenList,
                difficulty_function : str,
                competence_function : str
                #domain              : str,
                #proportion          : str,
                #fold                : str
            ):
                self.conllu_obj          = conllu_obj
                self.difficulty_function = difficulty_function
                self.competence_function = competence_function
                #self.domain              = domain
        ...
        my_instance = MyInstance(
            fields              = fields,
            conllu_obj          = conllu_obj,
            difficulty_function = difficulty_function,
            competence_function = competence_function 
            #domain              = domain,
